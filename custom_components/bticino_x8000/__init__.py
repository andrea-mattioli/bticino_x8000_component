...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.info(
        "Setting up Bticino X8000 integration with Optimized Core (Fix Boot Loop)"
    )
    ...
    if coordinator.update_interval == coordinator.cool_down_interval:
        _LOGGER.warning(
            "Skipping C2C Subscription due to active Rate Limit (Cool Down Mode)."
        )
        ...
        _LOGGER.info(
            "Registering C2C subscriptions for %s plants to URL: %s",
            len(plant_ids),
            webhook_url,
        )
        ...
        for plant_id in plant_ids:
            try:
                payload = {"EndPointUrl": webhook_url}
                ...
                if status in (200, 201):
                    ...
                    _LOGGER.warning(
                        "Failed to subscribe C2C for Plant %s: %s", plant_id, response
                    )
